...
object KairanSubtitleService {
            ...
            try {
                ...
                SubtitleStore.get(context, normalizeTitleForFile(title), episodeNumber, "kairan")
                    ...
                    ?.let {
                        ...
                        val syncedRecently = isAssetScanFresh(context, title, episodeNumber)
                ...
                links.forEachIndexed { index, link ->
                    ...
                    val asset = downloadGoogleDriveAsset(context, id, title, episodeNumber, index)
    ...
    private suspend fun findBlogPost(context: Context, title: String, episode: Int): String? {
        ...
        var match = KairanPostMatcher.findBestMatch(title, episode, posts)
        if (match == null) {
            ...
            if (refreshed.isNotEmpty()) {
                ...
                match = KairanPostMatcher.findBestMatch(title, episode, refreshed)
            }
        }
        if (match == null) {
    ...
    private fun hasKairanFonts(context: Context, subtitlePath: String): Boolean {
        val base = File(subtitlePath).nameWithoutExtension
        val safe = Regex("^(.*)_\\d+(?:_(?:direct|archive).*)?$")
            .find(base)?.groupValues?.getOrNull(1)
            ?: base.substringBeforeLast("_")
        val dir = File(context.filesDir, "$CACHE_DIR/fonts/$safe")
        return dir.isDirectory && dir.listFiles()?.any {
            it.isFile && it.extension.lowercase(Locale.ROOT) in setOf("ttf", "otf", "ttc")
        } == true
    }

    private fun assetScanKey(title: String, episode: Int): String = "${normalizeTitleForFile(title)}#$episode"

    private fun isAssetScanFresh(context: Context, title: String, episode: Int): Boolean {
        val prefs = context.getSharedPreferences(POST_CACHE_PREFS, Context.MODE_PRIVATE)
        if (prefs.getInt("asset_scan_version:${assetScanKey(title, episode)}", 0) != ASSET_SCHEMA_VERSION) return false
        val t = prefs.getLong("asset_scan:${assetScanKey(title, episode)}", 0L)
        return System.currentTimeMillis() - t < 6L * 60L * 60L * 1000L
    }

    private fun markAssetScan(context: Context, title: String, episode: Int) {
        context.getSharedPreferences(POST_CACHE_PREFS, Context.MODE_PRIVATE)
            .edit()
            .putLong("asset_scan:${assetScanKey(title, episode)}", System.currentTimeMillis())
            .putInt("asset_scan_version:${assetScanKey(title, episode)}", ASSET_SCHEMA_VERSION)
            .apply()
    }
